...
#import images
dir_images <- "~/Desktop/images" #;dir_images
...
```{r}
img_test_dir <- img_train_dir <- "~/Desktop/images/"
#img_name <- testname
...
feature <- function(img_dir,img_name,data_name = NULL){
  ...
  for(i in 1:n){
    ...
    img <- channel(img, mode="gray") #;display(img)
    ...
    img <- thresh(img, w = 50, h = 50, offset = 0.05) #;display(final_img)
...
#traindata <- feature(img_train_dir, trainname)
testdata <- feature(img_test_dir, testname)
efourier(testdata[[1]], nb.h = 10)
